...
def build_config_tabview(self):
    ...
    t_llm = "LLM Model settings" if config_manager.IS_ENGLISH else "大模型配置"
    t_emb = "Embedding settings" if config_manager.IS_ENGLISH else "向量嵌入配置"
    t_choose = "Config choose" if config_manager.IS_ENGLISH else "生成配置选择"
    t_proxy = "Proxy setting" if config_manager.IS_ENGLISH else "代理设置"

    self.ai_config_tab = self.config_tabview.add(t_llm)
    self.embeddings_config_tab = self.config_tabview.add(t_emb)
    self.config_choose = self.config_tabview.add(t_choose)

    # PenBo 增加代理功能支持
    self.proxy_setting_tab = self.config_tabview.add(t_proxy)
...
def build_ai_config_tab(self):
    ...
    def add_new_config():
        ...
        dialog = ctk.CTkInputDialog(
            text=get_text("new_config_prompt", "请输入新配置名称:"),
            title=get_text("new_config_title", "新增配置")
        )
        ...
        if new_name in self.loaded_config.get("llm_configs", {}):
            messagebox.showerror(get_text("error", "错误"), get_text("config_exists_err", "配置名称 '{}' 已存在!").format(new_name))
            return
        ...
        messagebox.showinfo(get_text("info", "提示"), get_text("new_config_success", "已成功创建新配置: {}").format(new_name))
    ...
    def delete_current_config():
        ...
        if selected_config in self.loaded_config.get("llm_configs", {}):
            if len(self.loaded_config["llm_configs"]) <= 1:
                messagebox.showerror(get_text("error", "错误"), get_text("keep_one_config_err", "至少需要保留一个配置!"))
                return
                
            confirm = messagebox.askyesno(
                get_text("confirm_delete_title", "确认删除"),
                get_text("confirm_delete_prompt", "确定要删除配置 '{}' 吗?\n此操作不可撤销!").format(selected_config)
            )
            if not confirm:
                return
            ...
            try:
                ...
                messagebox.showinfo(get_text("info", "提示"), get_text("delete_success", "已删除配置: {}，并已更新配置文件").format(selected_config))
            except Exception as e:
                messagebox.showerror(get_text("error", "错误"), get_text("save_config_fail", "保存配置文件失败: {}").format(str(e)))
        else:
            messagebox.showerror(get_text("error", "错误"), get_text("config_not_found", "未找到选中的配置!"))

    def save_current_config():
        """保存当前配置的修改到JSON文件"""
        config_name = self.interface_config_var.get()
        if config_name not in self.loaded_config.get("llm_configs", {}):
            messagebox.showerror(get_text("error", "错误"), get_text("config_not_exist", "配置不存在!"))
            return
        ...
        try:
            save_config(self.loaded_config, self.config_file)
            messagebox.showinfo(get_text("info", "提示"), get_text("config_save_success", "配置 {} 已保存并持久化到文件").format(new_name))
        except Exception as e:
            messagebox.showerror(get_text("error", "错误"), get_text("save_config_fail", "保存配置文件失败: {}").format(str(e)))
    ...
    def rename_current_config():
        ...
        if old_name not in self.loaded_config.get("llm_configs", {}):
            messagebox.showerror(get_text("error", "错误"), get_text("config_not_exist", "当前配置不存在!"))
            return
            
        dialog = ctk.CTkInputDialog(
            text=get_text("rename_config_prompt", "请输入新的配置名称 (原名称: {}):").format(old_name),
            title=get_text("rename_config_title", "重命名配置")
        )
        new_name = dialog.get_input()
        ...
        messagebox.showinfo(get_text("info", "提示"), get_text("rename_config_success", "配置已从 '{}' 重命名为 '{}'").format(old_name, new_name))
    ...
    # 配置选择控件
    create_label_with_help(self, self.ai_config_tab, get_text("current_config", "当前配置"), "interface_config", 0, 0)
    config_names = list(self.loaded_config.get("llm_configs", {}).keys())
    if not config_names:
        default_name = get_text("default_config", "默认配置")
        self.loaded_config["llm_configs"] = {
            default_name: {
                "id": str(uuid.uuid4()),
                "api_key": "",
    ...
    rename_btn = ctk.CTkButton(
        btn_frame, 
        text=get_text("btn_rename", "✏️ 重命名"), 
    ...
    row_start = 2
    # 1) API Key
    create_label_with_help(self, self.ai_config_tab, get_text("api_key", "API Key:"), "api_key", row_start, 0)
    ...
    # 2) Base URL
    create_label_with_help(self, self.ai_config_tab, get_text("base_url", "Base URL:"), "base_url", row_start+1, 0)
    ...
    # 3) 接口格式
    create_label_with_help(self, self.ai_config_tab, get_text("interface_format", "接口格式:"), "interface_format", row_start+2, 0)
    ...
    # 4) Model Name
    create_label_with_help(self, self.ai_config_tab, get_text("model_name", "模型名称:"), "model_name", row_start+3, 0)
    ...
    # 5) Temperature
    create_label_with_help(self, self.ai_config_tab, get_text("temperature", "Temperature:"), "temperature", row_start+4, 0)
    ...
    # 6) Max Tokens
    create_label_with_help(self, self.ai_config_tab, get_text("max_tokens", "Max Tokens:"), "max_tokens", row_start+5, 0)
    ...
    # 7) Timeout
    create_label_with_help(self, self.ai_config_tab, get_text("timeout", "Timeout (sec):"), "timeout", row_start+6, 0)
...
def build_embeddings_config_tab(self):
    ...
    # 1) Embedding API Key
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=get_text("emb_api_key", "Embedding API Key:"), tooltip_key="embedding_api_key", row=0, column=0, font=("Microsoft YaHei", 12))
    ...
    # 2) Embedding 接口格式
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=get_text("emb_interface", "Embedding 接口格式:"), tooltip_key="embedding_intexrface_format", row=1, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=get_text("emb_url", "Embedding Base URL:"), tooltip_key="embedding_url", row=2, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=get_text("emb_model", "Embedding Model Name:"), tooltip_key="embedding_model_name", row=3, column=0, font=("Microsoft YaHei", 12))
    ...
    # 5) Retrieval Top-K
    create_label_with_help(self, parent=self.embeddings_config_tab, label_text=get_text("emb_k", "Retrieval Top-K:"), tooltip_key="embedding_retrieval_k", row=4, column=0, font=("Microsoft YaHei", 12))
    ...
    # 添加测试按钮
    test_btn = ctk.CTkButton(self.embeddings_config_tab, text=get_text("test_emb", "测试配置"), command=self.test_embedding_config, font=("Microsoft YaHei", 12))
...
def build_config_choose_tab(self):
    ...
    create_label_with_help(self, parent=self.config_choose, label_text=get_text("arch_llm", "生成架构所用大模型"), tooltip_key="architecture_llm_config", row=0, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.config_choose, label_text=get_text("outline_llm", "生成大目录所用大模型"), tooltip_key="chapter_outline_llm_config", row=1, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.config_choose, label_text=get_text("draft_llm", "生成草稿所用大模型"), tooltip_key="prompt_draft_llm_config", row=2, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.config_choose, label_text=get_text("final_chapter_llm", "定稿章节所用大模型"), tooltip_key="final_chapter_llm_config", row=3, column=0, font=("Microsoft YaHei", 12))
    ...
    create_label_with_help(self, parent=self.config_choose, label_text=get_text("consistency_review_llm", "一致性审校所用大模型"), tooltip_key="consistency_review_llm_config", row=4, column=0, font=("Microsoft YaHei", 12))
    ...
    def save_config_choose():
        ...
        save_config_data_full = load_config(self.config_file)
        save_config_data_full["choose_configs"] = config_data
        save_config(save_config_data_full, self.config_file)
        messagebox.showinfo(get_text("info", "提示"), get_text("config_save_success", "配置已保存。").replace(" {}", ""))
    ...
    save_btn = ctk.CTkButton(
        self.config_choose, 
        text=get_text("save_choose", "保存配置"), 
    ...
    refresh_btn = ctk.CTkButton(
        self.config_choose, 
        text=get_text("refresh_choose", "刷新配置"), 
...
def build_proxy_setting_tab(self):
    ...
    # 代理启用开关
    create_label_with_help(self, self.proxy_setting_tab, get_text("proxy_enabled", "启用代理:"), "proxy_enabled", 0, 0)
    ...
    # 地址输入框
    create_label_with_help(self, self.proxy_setting_tab, get_text("proxy_address", "地址:"), "proxy_address", 1, 0)
    ...
    # 端口输入框
    create_label_with_help(self, self.proxy_setting_tab, get_text("proxy_port", "端口:"), "proxy_port", 2, 0)
    ...
    def save_proxy_setting():
        ...
        save_config(config_data, self.config_file)
        messagebox.showinfo(get_text("info", "提示"), get_text("config_save_success", "代理配置已保存。").replace(" {}", ""))
    ...
    save_btn = ctk.CTkButton(
        self.proxy_setting_tab,
        text=get_text("save_proxy", "保存代理设置"),
...
def load_config_btn(self):
    ...
    if cfg:
        ...
        self.log(get_text("load_config_success", "已加载配置。"))
    else:
        messagebox.showwarning(get_text("info", "提示"), get_text("load_config_fail", "未找到或无法读取配置文件。"))
...
def save_config_btn(self):
    ...
    if save_config(existing_config, self.config_file):
        messagebox.showinfo(get_text("info", "提示"), get_text("save_config_btn_success", "配置已保存至 config.json"))
        self.log(get_text("config_save_success", "配置已保存。").replace(" {}", ""))
    else:
        messagebox.showerror(get_text("error", "错误"), get_text("save_config_fail", "保存配置失败。"))