...
1.关键词组合逻辑：
-类型1：[人物]+[状态/秘密]（如"李火旺 心素 发作代价"）
-类型2：[事件]+[后果]（如"寺庙异变 队伍分裂"）
-类型3：[地点]+[规则/势力]（如"牛心村 坐忘道 误导"）
-类型4：[道具/功法]+[限制/代价]（如"铜钱剑 使用代价"）
...
请生成“静态角色档案与阵营档案”，不是当前章节状态。

输出规则：
1. 每个角色或阵营必须用二级标题开头，格式为：
   ## 主要角色：角色名
   ## 重要配角：角色名
   ## 阵营：阵营名
2. 主要角色和重要配角包含以下字段：
...
3. 阵营、组织、宗门或规则实体包含以下字段：
   ...
   - 与主线冲突
   - 可埋伏笔
4. 不要写“当前伤势、当前位置、当前情绪”等章节推进后的动态状态；这些属于 character_state.md。

要求：
仅返回可直接拆分保存的档案文本，不要解释任何内容。
"""
